...
func main() {
	...
	//config, err := gosdk.NewClientConfig(rpcURL, "fbc-101", gosdk.BroadcastSync, "0.000001fibo", 10000,
	//	0, "")
	//if err != nil {
	//	log.Fatal(err)
	//}
	...
	config, err := gosdk.NewClientConfig(rpcURL, "fbc-101", gosdk.BroadcastSync, "0.000001fibo", 450000,
		1.1, "0.000000000000000012fibo")
	if err != nil {
		log.Fatal(err)
	}
	...
	accInfo, err := cli.Auth().QueryAccount("fb1jyvuazl2nz0wd5v4zn62swg0x5pk38tefk3gcf")
	if err != nil {
		log.Fatal(err)
	}
...
func testTransfer(ibc exposed.Ibc, accountNum, sequenceNum uint64) {
	...
	res, err := ibc.Transfer(secp256k12.GenPrivKeySecp256k1(d), "channel-0", addr, "1000fibo", fees, "memo", client_types.Height{RevisionNumber: 101, RevisionHeight: 10000})
	if err != nil {
		log.Fatal(err)
	}
