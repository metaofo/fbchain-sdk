...
func TestStakingClient_Deposit(t *testing.T) {
	...
	res, err := mockCli.Staking().Deposit(fromInfo, passWd, "1024.1024fibo", memo, accInfo.GetAccountNumber(), accInfo.GetSequence())
	require.NoError(t, err)
	require.Equal(t, uint32(0), res.Code)
	...
	_, err = mockCli.Staking().Deposit(fromInfo, "", "1024.1024fibo", memo, accInfo.GetAccountNumber(), accInfo.GetSequence())
	require.Error(t, err)
}
...
func TestStakingClient_Withdraw(t *testing.T) {
	...
	res, err := mockCli.Staking().Withdraw(fromInfo, passWd, "1024.1024fibo", memo, accInfo.GetAccountNumber(), accInfo.GetSequence())
	require.NoError(t, err)
	require.Equal(t, uint32(0), res.Code)
	...
	_, err = mockCli.Staking().Withdraw(fromInfo, "", "1024.1024fibo", memo, accInfo.GetAccountNumber(), accInfo.GetSequence())
	require.Error(t, err)
}
