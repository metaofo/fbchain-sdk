...
const (
	...
	textProposalJSON               = `{"title":"Text Proposal","description":"text proposal description","proposal_type":"Text","deposit":"100fibo"}`
	paramsChangeProposalJSON       = `{"title":"Param Change Proposal","description":"param change proposal description","changes":[{"subspace":"staking","key":"MaxValidators","value":105}],"deposit":[{"denom":"fibo","amount":"100"}],"height":"1024"}`
	delistProposalJSON             = `{"title":"Delist Proposal","description":"delist proposal description","base_asset":"btc-000","quote_asset":"fibo","deposit":[{"denom":"fibo","amount":"100"}]}`
	communityPoolSpendProposalJSON = `{"title":"Community Pool Spend Proposal","description":"community pool spend description","recipient":"fb1qj5c07sm6jetjz8f509qtrxgh4psxkv3htfcuj","amount":[{"denom":"fibo","amount":"10.24"}],"deposit":[{"denom":"fibo","amount":"100"}]}`
	manageWhiteListProposalJSON    = `{"title":"Manage White List Proposal","description":"manage white list description","pool_name":"pool1","is_added":true,"deposit":[{"denom":"tfibo","amount":"100"}]}`
)
...
func TestGovClient_Deposit(t *testing.T) {
	...
	res, err := mockCli.Governance().Deposit(fromInfo, passWd, "100fibo", memo, 1,
		accInfo.GetAccountNumber(), accInfo.GetSequence())
	require.NoError(t, err)
	require.Equal(t, uint32(0), res.Code)

	_, err = mockCli.Governance().Deposit(fromInfo, passWd, "100fibo", memo, 0,
		accInfo.GetAccountNumber(), accInfo.GetSequence())
	require.Error(t, err)
	...
	_, err = mockCli.Governance().Deposit(fromInfo, "", "100fibo", memo, 1,
		accInfo.GetAccountNumber(), accInfo.GetSequence())
	require.Error(t, err)
	...
	mockCli.EXPECT().BuildAndBroadcast(
		fromInfo.GetName(), passWd, memo, gomock.AssignableToTypeOf([]sdk.Msg{}), accInfo.GetAccountNumber(), accInfo.GetSequence()).
		Return(sdk.TxResponse{}, errors.New("default error"))
	_, err = mockCli.Governance().Deposit(fromInfo, passWd, "100fibo", memo, 1,
		accInfo.GetAccountNumber(), accInfo.GetSequence())
	require.Error(t, err)
}
